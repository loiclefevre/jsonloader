...
public class MetadataIndex {
				...
				if (key.spatial) {
					final String SQLStatement = String.format(
							"create index %s on \"%s\" " +
									...
									"indextype is MDSYS.SPATIAL_INDEX parallel 8", collectionName + "$" + name, getDDLTableName(collectionName), key.columns.get(0).name);
	...
	private static final int UPPERCASE_PATTERN = 0;
	private static final int LOWERCASE_PATTERN = 1;
	private static final int SYMBOL_PATTERN = 2;
	private static final Pattern[] checkPatterns = new Pattern[3];

	static {
		checkPatterns[UPPERCASE_PATTERN] = Pattern.compile(".*[A-Z].*");
		checkPatterns[LOWERCASE_PATTERN] = Pattern.compile(".*[a-z].*");
		checkPatterns[SYMBOL_PATTERN] = Pattern.compile(".*[`~!@#$%^&*()\\-=+\\\\|\\[{\\]};:'\",<.>/?].*");
	}

	public static String getDDLTableName(String collectionName) {
       /* final boolean hasUpperChar = checkPatterns[UPPERCASE_PATTERN].matcher(collectionName).matches();
        final boolean hasLowerChar = checkPatterns[LOWERCASE_PATTERN].matcher(collectionName).matches();
        final boolean hasSymbolChar = checkPatterns[SYMBOL_PATTERN].matcher(collectionName).matches();

        if((hasUpperChar && hasLowerChar) || hasSymbolChar) {
			...
			try (Statement s = c.createStatement()) {
				...
				try {
					...
					s.execute(String.format("CREATE SEARCH INDEX %s$search_index ON \"%s\" (json_document) FOR JSON " +
							"PARAMETERS('DATAGUIDE OFF SYNC(MANUAL)')", collectionName, getDDLTableName(collectionName)));
